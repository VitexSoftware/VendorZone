...
if ($emailTo) {
    ...
    if (count($userDbFound)) {
        ...
        $oPage->addStatusMessage(sprintf(_('User %s not exists'),
                '<strong>'.$emailTo.'</strong>'));
    }
} else {
...
$row       = $oPage->container->addItem(new \Ease\Html\DivTag(null,
    ['class' => 'row']));
...
if (!$success) {
    ...
    $columnII->addItem(new \Ease\TWB\Well([_('Please check your mailbox for new password')
        , ' '._('and').' ', new \Ease\TWB\LinkButton('login.php?login='.$userLogin, _('Sign In'),
            'success')]));
}
