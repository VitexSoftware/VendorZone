 ...
 * @author vitex
 */
class FakturaVydana extends \VendorZone\WebHookHandler
{
    ...
    public function orderModulesForInvoiceItems()
    {
        ...
        if (count($prodCodes)) {
            $d     = dir("VendorZone/orderplugins");
            while (false !== ($entry = $d->read())) {
                if ($entry[0] == '.') {
                    continue;
                }
                $pluginName = str_replace('.php', '', $entry);
                $className  = '\\VendorZone\\VendorZone\\orderplugins\\'.$pluginName;

                /**
                 * @var \VendorZone\OrderPlugin OrderPlugin
                 */
