<?php
/**
 * vendorzone - Konvertor zálohových faktur do závazků.
 *
 ...
 * @author vitex
 */
class ZalohyZeSmluvDoZavazku extends \FlexiPeeHP\FlexiBeeRW
{
    /**
     *
     * @var array
    ...
    public function pripravSmlouvy()
    {
        ...
        if (count($smlouvy)) {
            foreach ($smlouvy as $id => $tmp) {
                $generated = $this->performRequest('dodavatelska-smlouva/'.$id.'/generovani-faktur.xml',
                    'PUT', 'xml');
    ...
    public function zkonvertujZalohyNaZavazky()
    {
        foreach ($this->zalohy as $id => $zaloha) {
            $this->zavazky[$id] = $this->convert($zaloha);
        }
    }
    ...
    public function ulozZavazky()
    {
        ...
        foreach ($this->zavazky as $id => $zavazek) {
            ...
            if ($this->lastResponseCode == 201) {
                ...
                $this->addStatusMessage(sprintf(_('Závazek %d vytvořen'),
                        $zavazekID));
        ...
        if (count($success)) {
            $this->addStatusMessage(sprintf(_('Bylo vygenerováno %s ostatních závazků z %s zálohových faktur'),
    ...
    public function uklidZpracovaneZalohy()
    {
        ...
        if (count($this->zalohy)) {
            ...
            $this->addStatusMessage(_('Zálohové faktury převedené na závazky byly smazány'),
                'success');
        }
    }
